...
export class ZoomPanPinch {
	...
	public animationRequestId: number = 0;
	...
	constructor(props: ReactZoomPanPinchProps) {
		...
		this.allowVerticalScrollThrough =
			Boolean(this.setup.panning.allowVerticalScrollThrough);
	}
	...
	onPanningStart = (event: MouseEvent): void => {
		...
		if (this.animation && this.setup.lockInteractionsDuringAnimation) {
			// console.log("[rzpp] ⚠️ onPanningStart aborted: animation in progress and interactions are locked", { animation: this });
			return;
		}
